...
class AniExr(object):
    ...
    def load(self, progress_bar):
        """
        loads the image layers
        :param progress_bar : the pyqt progress dialog object
        """
        ...
        self.__layer_images = self._build_images_for_layers(img_file, progress_bar, color_transform=True)
        img_file.close()
    ...
    def _build_images_for_layers(self, img_file, progress_bar, color_transform=True):
        """
        Make the images in memory for all the layers. Updates laoding progress as loads
        :param img_file: the exr image file handle - an OpenEXR.InputFile object
        :param progress_bar : the pyqt progress dialog object
        :param color_transform: whether to apply sRGB color transform
        :return: the layer images as a dictionary with key layer name and value the PIL Image Object
        """
        layer_images = {}
        ...
        for layer in self.layer_names:
            count += 1
            channels = self.layer_channel_names(layer)
            layer_image, error = self._layer_data_to_image(img_file, channels, self.size(), color_transform)
            layer_images[layer] = layer_image
            progress_bar.setValue(float(count) / float(layer_count) * 100.0)
    ...
    @staticmethod
    def _layer_data_to_image(img_file, channel_names, size, color_transform):
        """
        makes a image representation of a layer in an exr file
        :param img_file: the exr image file handle - an OpenEXR.InputFile object
        :param channel_names: the layer's channel names
        :param size: the size of the image
        :param color_transform: whether to apply sRGB color transform
        :return: a PIL Image object of the layer, error message if an error occured
        """
        ...
        try:
            ...
            return Image.merge("RGB", rgb8), None
        except TypeError:
            return None, "Could not read layer: {0}, {1}, {2}.".format(r_channel, g_channel, b_channel)
...
class AniExrViewerGui(QtWidgets.QMainWindow):
    ...
    def _load_image(self, exr_img_path):
        ...
        prg = QtWidgets.QProgressDialog("Loading Exr Layers", "Cancel", 0, 100, self)
        prg.show()
        QtWidgets.QApplication.processEvents()
        self.exr_image.load(prg)
        prg.hide()
